...
starting with the first row with an empty first box, the codes are filled in diagonally up and to the right.
...
At row R=1 and column C, wrap around to column 1 and row C+1.
Otherwise, go to row R-1 and column C+1.
...
def print_pattern(pattern: dict, sleep: float = 0.1):
    time.sleep(sleep)
    os.system("clear")
...
def compute_code(
    ...
    while row != target_row or col != target_column:
        row, col = (
            col + 1 if row == 1 else row - 1,
            1 if row == 1 else col + 1,
        )
        code = (code * 252533) % 33554393
        # pattern[row] = pattern.get(row, []) + [str(code)]
        # print_pattern(pattern)
...
def solve_part1():
    print(
        f"Part 1:\nThe code to start the weather machine is: {compute_code(2981, 3075)}!\n"
    )
